...
MISSING_THRESHOLD = 0.5 #delete all data columns missing more than this proportion of entries
...
def housing_clean():
  ...
  for col in list(combi.columns):
    ...
    if(missing_proportion >= MISSING_THRESHOLD) and  (col != 'SalePrice'): #drop columns with too many missing elements
      print("Dropping column '%s' due to %.1f%% missing values" % (col,missing_proportion*100))
      droplist.append(col)

  #drop columns (except SalePrice, it will automatically have NA's due to the inclusion of the test set)
  combi.drop(droplist)

  print("Filling in remaining missing values..")
