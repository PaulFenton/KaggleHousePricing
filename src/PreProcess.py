class PreProcess():
  ...
  def fillMissingData(data):
    ...
    cleanData["GarageCars"]cleanData["GarageCond"] = data["GarageCond"].fillna("None") = data["GarageCars"].fillna(0)
    ...
    cleanData["KitchenQual"] = data["KitchenQual"].fillna("RL")
    # MasVnrArea: for now, fill in the 23 values with the mean TODO: get more sophisticated here?
    mean = data["MasVnrArea"].mean()
    cleanData["MasVnrArea"] = data["MasVnrArea"].fillna(mean)
    # MasVnrType: for now, fill in the 24 values with the mean TODO: get more sophisticated here?
    mean = data["MasVnrArea"].mean()
    cleanData["MasVnrArea"] = data["MasVnrArea"].fillna("None")
